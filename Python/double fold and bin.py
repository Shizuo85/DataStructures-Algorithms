...
def resolve(d, lst):
    ...
    for k,v in d.items():
        assert (type(k)==str and len(k)==1 and (ord(k) in range(97, 123) or ord(k) in range(65, 91)) or type(k)==int and k>-1)
        assert (type(v)==str and len(v)==1 and (ord(v) in range(97, 123) or ord(v) in range(65, 91)) or type(v)==int and v>-1)
    for i in range(len(lst)):
        assert (type(lst[i])==str and len(lst[i])==1 and (ord(lst[i]) in range(97, 123) or ord(lst[i]) in range(65, 91)) or (type(lst[i])==int and lst[i]>-1))
        dict1={}
